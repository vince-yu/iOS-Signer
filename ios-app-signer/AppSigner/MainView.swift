...
class MainView: NSView, URLSessionDataDelegate, URLSessionDelegate, URLSessionDownloadDelegate {
    ...
    @objc let xattrPath = "/usr/bin/xattr"
            ...
            for file in files {
                ...
                if extensions.contains(file.pathExtension) {
                    if file.pathExtension != "" || file == "IpaSecurityRestriction" {
                        ...
                        //NSLog("couldnt find: %@", file)signEgg
                    }
...
//                        }
                ...
                //MARK:官方说这是出于安全起见而在iOS 10, macOS Sierra, watchOS 3, and tvOS 10上做的强制措施。代码签名不再允许bundle包中的任何文件有包含资源分支(resource fork)或Finder信息。
                //用以下命令移除所有文件的扩展属性
                let resut = Process().execute(xattrPath, workingDirectory: nil, arguments: ["-cr",appBundlePath]);
